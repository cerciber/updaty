...
async function getDataFromInternet({
  ...
  for (const responseSearch of responseSearches.message) {
    for (let i = 0; i < config.IA_ITERATIONS_PER_PAGE; i++) {
      const finalPromt =
        "Buscame '" +
        promt +
        "' solo basado en el siguiente texto:\n\n" +
  ...
  for (let i = 0; i < config.IA_ITERATIONS_ON_BAD_FORMAT; i++) {
    ...
    let conclutionPrompt =
      "Sólo con base en las siguientes respuestas dame el resultado mas probable para '" +
      promt +
    ...
    if (config.LOGS) {
      console.log(
        "  - Conclusion intent: " +
          (i + 1) +
          ": " +
          conclusionResponseChatGPT.message.replace(/[\n\r]/g, ""),
      );
    }
    ...
    try {
      ...
      const json_obj = JSON.parse(
        regex.exec(conclusionResponseChatGPT.message)[0],
      );
